    ...
    try:
        if path.endswith('.geo.json'):
            with open(path, 'r') as json_file:
                ...
                d = datetime.utcfromtimestamp(int(temp_json["Timestamp"]))
                return {
                    "Latitude": temp_json["location"]["lat"],
                    "Longitude": temp_json["location"]["lng"],
                    "Altitude": 10,
                    "Updated": d.strftime('%Y-%m-%dT%H:%M:%S.%f')
                }
        elif path.endswith('.gps.json'):
            with open(path, 'r') as json_file:
                temp_json = json.load(json_file)
                return {
                    "Latitude": temp_json["Latitude"],
                    "Longitude": temp_json["Longitude"],
                    "Altitude": temp_json["Altitude"],
                    "Updated": temp_json["Updated"]
                }
        else:
            with open(path, 'r') as json_file:
